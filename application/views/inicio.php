
        <!-- Ligas -->
        <section class="ligas">            
            <div class="container ">                    
                <div class="row header-liga text-center">
                    <img src="<?= assets()?>images/ligas/liga_cod_netllar.png" class="rounded mx-auto d-block">
                </div>
                <div class="row body-liga">
                    <div class="container-fluid">
                        <div class="row competiciones justify-content-around ">
                            
                            <div class="col-sm-12 col-md-12 col-lg-8 competicion ">
                                <div class="container-fluid text-center">
                                    <div >
                                        <div class="row header-competicion">
                                            <div class="col">
                                                <img src="<?= assets()?>images/ligas/codbo4-logo1.png">
                                                <!--<h3>Call Of Duty Black Ops 4</h3>-->
                                            </div>
                                        </div>
                                        <div class="row body-competicion">
                                            <div class="col-6">
                                                <h3> 500€ </h3>
                                                <img class="incono_h3 sombra-png-blanca" src="<?= assets()?>images/iconos/medalla-de-oro.png">
                                                
                                            </div>
                                            <div class="col-6">
                                                <h3> 250€ </h3>
                                                <img class="incono_h3 sombra-png-blanca" src="<?= assets()?>images/iconos/segundo.png">
                                                
                                            </div>
                                            <div class="col-12">                                                      
                                                <img class="incono_h3" src="<?= assets()?>images/iconos/Torneo_png.png">
                                                <h3> 5 vs 5 </h3>
                                            </div>
                                        </div>
                                        <div class="row footer-competicion">
                                            <div class="col">

                                                <a href="<?php echo site_url('LigaCOD');?>">
                                                    <img style="background-color: #fff" class="incono_h3" src="<?= assets()?>images/iconos/plus.png">
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <!--<div class="col-6 competicion ">
                                <div class="container-fluid text-center">
                                    <div >
                                        <div class="row header-competicion">
                                            <div class="col">
                                                <img src="<?= assets()?>images/cs-go.png">
                                                <h3>Call Of Duty Black Ops 4</h3>
                                            </div>
                                        </div>
                                        <div class="row body-competicion">
                                            <div class="col">
                                                <h3><img class="incono_h3" src="<?= assets()?>images/iconos/Concurso_png.png"> 500€ </h3>
                                                <h3><img class="small_incono_h3" src="<?= assets()?>images/iconos/Concurso_png.png"> 250€ </h3>
                                                <h3><img class="incono_h3" src="<?= assets()?>images/iconos/Torneo_png.png"> 5 vs 5 </h3>
                                            </div>
                                        </div>
                                        <div class="row footer-competicion">
                                            <div class="col">
                                                <h3><img class="incono_h3" src="<?= assets()?>images/iconos/Información_png.png"> Detalles </h3>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>-->

                        </div>                   
                    </div>                        
                </div>                    
            </div>          

           
        
        </section>
        <!-- Galeries -->
        <section class="galeria">
            <div class="container">
                <div class="row header-galeria text-center">
                    <div class="col">
                        <h3> Galeria </h3>
                    </div>
                </div>
                <div class="row body-galeria">
                    <!-- LightWidget WIDGET -->

                    <div class="col">
                        <script src="https://cdn.lightwidget.com/widgets/lightwidget.js"></script><iframe src="//lightwidget.com/widgets/3a6479b2919e511fa6e3274dacb39363.html" scrolling="no" allowtransparency="true" class="lightwidget-widget" style="width:100%;border:0;overflow:hidden;"></iframe>
                    </div>

                </div>
                
            </div>
        </section>      
        <!-- Videos -->
        <section class="videos">
            <div class="container">
                <div class="row header-videos text-center">
                    <div class="col">
                        <h3> Videos </h3>
                    </div>
                </div>
                <div class="row body-videos">
                    <div class="col-sm-12 col-md-12 col-lg-6">
                        <iframe width="100%" height="315" src="https://www.youtube.com/embed/uhLNxnw6n6E" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                    </div>
                    <div class="col-sm-12 col-md-12 col-lg-6">
                        <iframe width="100%" height="315" src="https://www.youtube.com/embed/TwxjcvxOhtE" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                    </div>
                </div>
            </div>
        </section>
        <!-- 
            Ranking 
       
        <section>
          <div class="container">
              <div class="row">
                  
              </div>
          </div>
        </section>
         -->


